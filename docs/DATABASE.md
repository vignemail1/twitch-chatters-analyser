...
**Limitation** : Maximum **10 sessions sauvegardées** par utilisateur. Les plus anciennes sont automatiquement supprimées.
...
docker exec -it twitch-chatters-db mariadb -u root -p
USE twitch_chatters;
SOURCE /path/to/001_limit_saved_sessions.sql;
```
...
### sessions
- `idx_sessions_user` : Recherche par utilisateur
- `idx_sessions_status` : Filtrage par statut
- `idx_sessions_user_status` : Combo user + status (pour `getActiveSessionUUID`)

### captures
- `idx_captures_session` : Recherche par session
- `idx_captures_broadcaster` : Filtrage par broadcaster
- `idx_captures_session_captured` : Tri temporel par session

### capture_chatters
- `idx_capture_chatters_capture` : Recherche par capture
- `idx_capture_chatters_user` : Recherche par user Twitch
- `idx_capture_chatters_capture_user` : Dédoublonnage

### jobs
- `idx_jobs_status_created` : Polling worker (tri par statut + date)
...
```bash
# Backup complet
docker exec twitch-chatters-db mariadb-dump -u root -p twitch_chatters > backup-$(date +%Y%m%d-%H%M%S).sql

# Backup sans les jobs (plus léger)
docker exec twitch-chatters-db mariadb-dump -u root -p twitch_chatters \
  --ignore-table=twitch_chatters.jobs \
  --ignore-table=twitch_chatters.audit_logs \
  > backup-minimal-$(date +%Y%m%d-%H%M%S).sql
```
...
Mots de passe stockés dans `.env` (non commité) :
...
MariaDB n'est **pas exposé** en dehors du réseau Docker (pas de `ports:` dans docker-compose.yml).
